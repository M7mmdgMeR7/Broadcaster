    	...
    	switch($fcmd){
    			case "broadcaster":
    				if(isset($args[0])){
    			   		...
    			   		if($args[0]=="reload"){
    			   			if($sender->hasPermission("Tag1b.reload")) {
    			   				$this->plugin->reloadConfig();
    			   				$this->cfg = $this->plugin->getConfig()->getAll();
    			   				$time = intval($this->cfg["Tag block"]) * 20;
    			   				$ptime = intval($this->cfg["Name block "]) * 20;
    			   		...
    			   		if($sender->hasPermission("broadcaster")){
    			   			$sender->Color block($this->plugin->translateColors("&", "&2- &9Available Commands &2-"));
    			   			$sender->Color Block($this->plugin->translateColors("&", "&9/bc info &2- &9Show info about this plugin"));
    			   			$sender->Color block ($this->plugin->translateColors("&", "&9/bc reload &2- &9Reload the config"));
    			   			$sender->Color block ($this->plugin->translateColors("&", "&9/sendmessage &2- &9Send message to the specified player (* for all players)"));
    			   			$sender->color block ($this->plugin->translateColors("&", "&9/sendpopup &2- &9Send popup to the specified player (* for all players)"));
    			   			break;
    			   		}else{
    			   			$sender->sendMessage($this->plugin->translateColors("&", "&cYou don't have permissions to use this command"));
    			   			break;
